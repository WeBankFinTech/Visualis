...
public class ScriptUtils {
                    ...
                    switch (key) {
                        case "groups":
                            ...
                            if (groupMirror.isArray()) {
                                ...
                                values.forEach(v -> groups.add(getRealColumn(String.valueOf(v))));
                            }
                            break;
                        case "aggregators":
                            ...
                            if (aggregatorsMirror.isArray()) {
                                ...
                                values.forEach(v -> {
                                    ...
                                    Aggregator aggregator = new Aggregator(getRealColumn(String.valueOf(agg.get("column"))), String.valueOf(agg.get("func")));
                        ...
                        case "orders":
                            ...
                            if (ordersMirror.isArray()) {
                                ...
                                values.forEach(v -> {
                                    ScriptObjectMirror agg = (ScriptObjectMirror) v;
                                    Order order = new Order(getRealColumn(String.valueOf(agg.get("column"))), String.valueOf(agg.get("direction")));
                                    orders.add(order);
                                });
                            }
                            break;
                        case "filters":
                            ...
                            if (o instanceof String) {
                                ...
                                if (filterMirror.isArray() && filterMirror.size() > 0) {
                                    ...
                                    values.forEach(v -> {
                                        filters.add(getRealColumn(String.valueOf(v)));
                                    });
                                }
                            }
                            break;
                        ...
                        case "params":
                            ...
                            if (paramsMirror.isArray()) {
                                ...
                                values.forEach(v -> {
                                    ScriptObjectMirror agg = (ScriptObjectMirror) v;
                                    Param param = new Param(getRealColumn(String.valueOf(agg.get("name"))), String.valueOf(agg.get("value")));
                                    params.add(param);
                                });
                            }
                            break;
                        case "nativeQuery":
                            ...
                            nativeQuery = nativeQuery == null ? false : nativeQuery;
                            break;
                        case "view":
                            if (vsom.get(key) instanceof Integer) {
                                break;
                            }
                            virtualView = JSONObject.parseObject(JSONObject.toJSONString(vsom.get(key)), VirtualView.class);
                            break;
                    }
                }

                return new ViewExecuteParam(Lists.newArrayList(groups), aggregators, orders, filters, virtualView, params, cache, expired, nativeQuery);
            }
    ...
    private static String getRealColumn(String columnName) {
        if (columnName.contains("@Visualis@")) {
            columnName = org.apache.commons.lang.StringUtils.substringBefore(columnName, "@Visualis@");
        }
        return columnName;
    }
