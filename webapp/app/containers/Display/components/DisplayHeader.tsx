...
export class DisplayHeader extends React.Component<IDisplayHeaderProps, IDisplayHeaderStates> {
  ...
  private onWidgetsSelectDone = (selectedWidgets, fieldValues) => {
    ...
    selectedWidgets.forEach((item) => {
      this.addSecondaryGraph(SecondaryGraphTypes.Label, item.name, item.name)()
    })
  }

  private addSecondaryGraph = (secondaryGraphType: SecondaryGraphTypes, name?: string, text?: string) => () => {
    if (this.props.layers && this.props.layers.length >= MAX_LAYER_COUNT) return message.warning(`当前最多只支持添加${MAX_LAYER_COUNT}个图层！`, 5);
    const title = (slideSettings[secondaryGraphType] as any).title
    const params = this.getDefaultSetting(GraphTypes.Secondary, secondaryGraphType)
    if (text) params.contentText = text
